...
class FingersCloseTogetherRule:
    def __init__(self, hand, detector):
        self.hand = hand
        self.detector = detector
...
class FourFingersCloseTogether():
    def __init__(self, hand, detector):
        self.hand = hand
        self.detector = detector
        self.rules = [FourFingersUpRule(hand, detector), FingersCloseTogetherRule(hand, detector)]

    def isShowing(self):
        for rule in self.rules:
            if not rule.passes(rule):
                return False
        print('four fingers close together')
        return True
